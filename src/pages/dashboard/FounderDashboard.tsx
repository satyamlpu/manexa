...
const FounderDashboard = () => {
  ...
  const [stats, setStats] = useState({
    ...
    pendingFees: 0, pendingSalaries: 0,
    feeCollectedToday: 0,
  });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setStats({
        ...
        totalParents: parentRolesRes.count || 0,
        todayPresentCount: presentToday,
        todayAbsentCount: totalToday - presentToday,
        todayAttendanceRate: rate,
        pendingTasks: tasksRes.count || 0,
        pendingFees: pendingFeeCount,
        pendingSalaries: pendingSalCount,
        feeCollectedToday: paidFees,
      });

      // Fee pie chart
      setFeeChart([
        { name: "Collected", value: paidFees },
        { name: "Pending", value: pendingFeeAmount },
      ]);
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const { data: dayData } = await supabase
          .from("attendance").select("status").eq("institution_id", instId).eq("date", dateStr);
        const present = dayData?.filter(a => a.status === "Present").length || 0;
        const absent = (dayData?.length || 0) - present;
        days.push({
          date: d.toLocaleDateString("en", { weekday: "short" }),
          present, absent,
          rate: dayData?.length ? Math.round((present / dayData.length) * 100) : 0,
        });
      }
      ...
      if (pendingFeeCount > 0) newAlerts.push({ type: "yellow", message: `${pendingFeeCount} student fee${pendingFeeCount > 1 ? "s" : ""} are pending` });
      if (pendingSalCount > 0) newAlerts.push({ type: "red", message: `${pendingSalCount} teacher salary payment${pendingSalCount > 1 ? "s" : ""} pending` });
      if ((tasksRes.count || 0) > 5) newAlerts.push({ type: "yellow", message: `${tasksRes.count} tasks are still pending` });
      setAlerts(newAlerts);
